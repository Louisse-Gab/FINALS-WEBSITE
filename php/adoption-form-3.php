  ...
  <main class="container mx-auto px-6 py-12">
    <form action="" method="post" class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
      <input type="hidden" name="id"
        value="<?php echo isset($_POST['id']) ? htmlspecialchars($_POST['id']) : (isset($_GET['id']) ? htmlspecialchars($_GET['id']) : ''); ?>">
      ...
      <label class="block text-sm font-medium text-gray-700 mb-3">
        Who do you want to adopt from our adoptables?
      </label>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <?php while ($pet = $result->fetch_assoc()): ?>
                <div class="flex items-center">
                  <input type="checkbox" id="pet<?= htmlspecialchars($pet['id']) ?>" name="desiredPet[]"
